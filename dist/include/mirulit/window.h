...
/* Callback по умолчанию для изменения размера окна */
static void default_framebuffer_size_callback(GLFWwindow* window, int width, int height)
{
    (void)window; /* Неиспользуемый параметр */
    glViewport(0, 0, width, height);
    window_width = width;
    window_height = height;
}

/* Callback по умолчанию для обработки ошибок GLFW */
static void error_callback(int error, const char* description)
{
    fprintf(stderr, "GLFW Error %d: %s\n", error, description);
}

/* Инициализация GLFW, GLAD и создание окна */
bool window_init(int width, int height, const char* title)
{
    ...
    /* Установка callback для изменения размера */
    glfwSetFramebufferSizeCallback(main_window, default_framebuffer_size_callback);
    
    /* Загрузка указателей на функции OpenGL через GLAD */
...
/* Получение указателя на окно GLFW */
GLFWwindow* window_get_handle(void)
{
    return main_window;
}
...
/* Получение времени в секундах с инициализации GLFW */
double window_get_time(void)
{
    return glfwGetTime();
}
...
/* Включение/выключение очистки цветового буфера */
void window_set_clear_color_enabled(bool enabled)
{
    clear_color_enabled = enabled;
}

/* Установка callback для изменения размера фреймбуфера */
void window_set_framebuffer_size_callback(GLFWframebuffersizefun callback)
{
    if (main_window) {
        if (callback == NULL) {
...
/* Установка callback для обработки клавиатуры */
void window_set_key_callback(GLFWkeyfun callback)
{
    if (main_window) {
...
/* Установка callback для обработки мыши */
void window_set_mouse_callback(GLFWcursorposfun callback)
{
    if (main_window) {
...
/* Установка callback для обработки прокрутки */
void window_set_scroll_callback(GLFWscrollfun callback)
{
    if (main_window) {
