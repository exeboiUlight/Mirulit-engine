...
typedef struct {
    float m[4]; /* [0][0], [1][0], [0][1], [1][1] */
} mat2;

/* Матрица 3x3 в column-major порядке */
typedef struct {
    float m[9]; /* [0][0], [1][0], [2][0], [0][1], [1][1], [2][1], [0][2], [1][2], [2][2] */
} mat3;

/* Матрица 4x4 в column-major порядке */
typedef struct {
    float m[16]; /* [0][0], [1][0], [2][0], [3][0], ... */
} mat4;
...
mat4 mat4_inverse(mat4 m)
{
    ...
    if (fabsf(det) < FLOAT_EPSILON) {
        return mat4_identity(); /* Нельзя инвертировать */
    }
    
    float inv_det = 1.0f / det;
    
    /* Вычисляем обратную матрицу через алгебраические дополнения */
